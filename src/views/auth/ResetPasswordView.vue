...
<template>
  <div class="flex items-center justify-center min-h-screen bg-gray-100">
    <div class="w-full max-w-md p-8 space-y-6 bg-white rounded-lg shadow-md">
      <h2 class="text-2xl font-bold text-center">Réinitialiser votre mot de passe</h2>

      <div v-if="!token || error" class="p-4 bg-red-50 text-red-700 rounded-md text-center">
        <p>{{ error || 'Token invalide.' }}</p>
        <RouterLink to="/login" class="mt-4 inline-block font-medium text-red-700 hover:underline">Retour à la connexion</RouterLink>
      </div>

      <div v-else-if="message" class="p-4 bg-green-50 text-green-800 rounded-md text-center">
        <p>{{ message }}</p>
        <RouterLink to="/login" class="mt-4 inline-block font-medium text-green-700 hover:underline">Se connecter</RouterLink>
      </div>
      ...
      <form v-else @submit.prevent="handleResetPassword" class="space-y-6">
        <div>
          ...
          <input v-model="password" type="password" id="new-password" required class="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
        </div>
        <div>
          <label for="confirm-password" class="block text-sm font-medium text-gray-700">Confirmer le mot de passe</label>
          <input v-model="confirmPassword" type="password" id="confirm-password" required class="w-full px-3 py-2 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500">
        </div>
        <div>
