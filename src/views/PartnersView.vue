  ...
  <div class="bg-gray-50">
    <div class="max-w-7xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8">
      <div class="text-center">
        <h2 class="text-base font-semibold text-green-700 tracking-wide uppercase">Nos Alliés</h2>
      ...
      <div class="mt-20">
        <div v-if="isLoading">
          <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            <div v-for="i in 6" :key="i" class="bg-white rounded-lg shadow-lg animate-pulse">
        ...
        <div v-else-if="error" class="text-center bg-red-50 p-4 rounded-md">
          <p class="text-red-700">{{ error }}</p>
        </div>

        <div v-else-if="partners.length > 0" class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
          <div v-for="partner in partners" :key="partner.id" class="flex flex-col bg-white rounded-lg shadow-lg overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
            <div class="flex-shrink-0 h-48 bg-gray-100 flex items-center justify-center">
              <img v-if="partner.logoUrl" class="h-full w-full object-contain p-4" :src="partner.logoUrl" :alt="'Logo de ' + partner.name">
            ...
            <div class="flex-1 p-6 flex flex-col justify-between">
              <div>
                ...
                <p class="mt-3 text-base text-gray-500 line-clamp-4">{{ partner.description }}</p>
              </div>
              <div class="mt-6">
                <a v-if="partner.website" :href="partner.website" target="_blank" rel="noopener noreferrer" class="text-base font-medium text-green-700 hover:text-green-600">
      ...
      <hr class="text-gray-200 mt-10">

      <section class="bg-green-700 rounded-lg mt-24">
        <div class="max-w-2xl mx-auto text-center py-16 px-4 sm:py-20 sm:px-6 lg:px-8">
          ...
          <RouterLink to="/devenir-partenaire" class="mt-8 w-full inline-flex items-center justify-center px-5 py-3 border border-transparent text-base font-medium rounded-md text-green-600 bg-white hover:bg-green-50 sm:w-auto">
            Devenir partenaire
          </RouterLink>
        </div>
      </section>
