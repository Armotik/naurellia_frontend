      ...
      <section class="bg-gray-50 py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="text-center">
            <h2 class="text-base font-semibold text-green-700 tracking-wide uppercase">Un Partenariat de Valeurs</h2>
          ...
          <div class="mt-16 grid grid-cols-1 md:grid-cols-3 gap-12 text-center">
            <div>
              <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-green-100 text-green-700 shadow-lg">
              ...
              <h3 class="mt-5 text-lg font-medium text-gray-900">Visibilité Ciblée</h3>
              <p class="mt-2 text-base text-gray-500">Touchez une audience qualifiée, sensible aux valeurs de l'écologie et du tourisme responsable.</p>
            </div>
            <div>
              <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-green-100 text-green-700 shadow-lg">
              ...
              <h3 class="mt-5 text-lg font-medium text-gray-900">Image de Marque Positive</h3>
              <p class="mt-2 text-base text-gray-500">Associez votre nom à une initiative locale, moderne et respectueuse de l'environnement rétais.</p>
            </div>
            <div>
              <div class="mx-auto flex items-center justify-center h-16 w-16 rounded-full bg-green-100 text-green-700 shadow-lg">
              ...
              <h3 class="mt-5 text-lg font-medium text-gray-900">Soutien à la Mission</h3>
              <p class="mt-2 text-base text-gray-500">Participez activement à la protection du patrimoine naturel et culturel de l'île.</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div class="mt-12">
            <form @submit.prevent="handleSubmit" class="space-y-6">
              <div class="grid grid-cols-1 gap-y-6 gap-x-8 sm:grid-cols-2">
                <div>
                  <label for="company-name" class="block text-sm font-medium text-gray-700">Nom de votre structure</label>
                  <input v-model="formData.companyName" type="text" name="company-name" id="company-name" required class="mt-1 py-3 px-4 block w-full shadow-sm focus:ring-green-500 focus:border-green-500 border-gray-300 rounded-md">
                </div>
                <div>
                  <label for="business-type" class="block text-sm font-medium text-gray-700">Type d'activité</label>
                  <select v-model="formData.businessType" id="business-type" name="business-type" required class="mt-1 block w-full py-3 px-4 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500">
              ...
              <div class="grid grid-cols-1 gap-y-6 gap-x-8 sm:grid-cols-2">
                <div>
                  <label for="contact-name" class="block text-sm font-medium text-gray-700">Votre nom (contact)</label>
                  <input v-model="formData.contactName" type="text" name="contact-name" id="contact-name" required class="mt-1 py-3 px-4 block w-full shadow-sm focus:ring-green-500 focus:border-green-500 border-gray-300 rounded-md">
                </div>
                <div>
                  <label for="email" class="block text-sm font-medium text-gray-700">Email de contact</label>
                  <input v-model="formData.email" id="email" name="email" type="email" required class="mt-1 py-3 px-4 block w-full shadow-sm focus:ring-green-500 focus:border-green-500 border-gray-300 rounded-md">
                </div>
              </div>
              <div>
                <label for="phone" class="block text-sm font-medium text-gray-700">Téléphone <span class="text-gray-400">(Optionnel)</span></label>
                <input v-model="formData.phone" type="tel" name="phone" id="phone" class="mt-1 py-3 px-4 block w-full shadow-sm focus:ring-green-500 focus:border-green-500 border-gray-300 rounded-md">
              </div>
              <div>
                <label for="website" class="block text-sm font-medium text-gray-700">Site web <span class="text-gray-400">(Optionnel)</span></label>
                <input v-model="formData.website" type="url" name="website" id="website" placeholder="https://..." class="mt-1 py-3 px-4 block w-full shadow-sm focus:ring-green-500 focus:border-green-500 border-gray-300 rounded-md">
              </div>
              <div>
                <label for="message" class="block text-sm font-medium text-gray-700">Votre message et vos motivations</label>
                <textarea v-model="formData.message" id="message" name="message" rows="5" required placeholder="Parlez-nous de vous, de vos valeurs, et de la manière dont votre activité s'inscrit dans une démarche de tourisme durable..." class="mt-1 py-3 px-4 block w-full shadow-sm focus:ring-green-500 focus:border-green-500 border border-gray-300 rounded-md"></textarea>
              </div>
              <div v-if="successMessage" class="p-4 bg-green-50 text-green-700 rounded-md">{{ successMessage }}</div>
              <div v-if="errorMessage" class="p-4 bg-red-50 text-red-700 rounded-md whitespace-pre-line">{{ errorMessage }}</div>
              <div>
                <button type="submit" :disabled="isLoading" class="w-full inline-flex items-center justify-center px-6 py-3 border border-transparent rounded-md shadow-sm text-base font-medium text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 disabled:bg-gray-400">
                  {{ isLoading ? 'Envoi en cours...' : 'Envoyer ma demande de partenariat' }}
                </button>
              </div>
            </form>
          </div>
