  ...
  <div class="relative h-[calc(100vh-64px)] bg-gray-100 dark:bg-gray-900 overflow-hidden">
    ...
    <div class="md:flex h-full">
      ...
      >
        ...
        <div v-if="authStore.isAdmin" class="mt-8 pt-6 border-t dark:border-gray-700">
          <h3 class="text-lg font-bold text-gray-800 dark:text-white">Administration</h3>
          <div class="mt-4 p-3 bg-gray-100 dark:bg-gray-700 rounded-lg space-y-4">
            <label for="edit-mode-toggle" class="flex items-center justify-between cursor-pointer">
              <span class="text-gray-700 dark:text-gray-300 font-medium">Mode Édition</span>
              <div class="relative">
                ...
                <div class="dot absolute left-1 top-1 bg-white dark:bg-gray-300 w-6 h-6 rounded-full transition-transform"></div>
              </div>
            </label>
            <p v-if="isEditMode" class="text-sm text-green-700 dark:text-green-500">Le mode édition est actif.</p>
            <button @click="openPoiModal(null)" class="w-full px-4 py-2 bg-green-600 dark:bg-green-700 text-white rounded-md hover:bg-green-700 dark:hover:bg-green-600 text-sm font-semibold">
              Créer un point
            </button>
          </div>
        </div>
    ...
    <div v-if="isPoiModalOpen" class="fixed inset-0 bg-black/60 flex items-center justify-center z-[1001]" @click.self="closePoiModal">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-2xl dark:shadow-black/60 p-6 md:p-8 w-full max-w-lg mx-4">
        <h2 class="text-2xl font-bold mb-6 text-gray-800 dark:text-white">{{ currentPoi.id ? 'Modifier le Point d\'Intérêt' : 'Nouveau Point d\'Intérêt' }}</h2>
...
<style>
/* Styles pour les popups Leaflet en mode sombre */
...
/* Personnalisation de la prose pour le thème sombre */
.dark .prose {
