  ...
  <div class="bg-gray-50">
    <div class="max-w-7xl mx-auto py-16 px-4 sm:px-6 lg:px-8">
      <div class="text-center">
        <h1 class="text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl">Le Blog de Naurellia</h1>
      ...
      <div v-else-if="error" class="mt-12 text-center bg-red-50 p-4 rounded-md"><p class="text-red-700">{{ error }}</p></div>

      <div v-else class="mt-12">
        <div v-if="featuredArticles.length > 0" class="mb-20">
          <h2 class="text-3xl font-bold tracking-tight text-gray-900 mb-8">À la une</h2>
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <article v-if="featuredArticles[0]" class="lg:col-span-2 relative rounded-xl overflow-hidden border border-gray-200 group bg-white">
              <div class="h-96 bg-gray-200"><svg class="h-full w-full text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Z"/></svg></div>
              <div class="p-8">
                ...
                <h3 class="text-2xl font-bold leading-tight mt-2"><RouterLink :to="`/blog/articles/${featuredArticles[0].slug}`" class="hover:text-green-700"><span class="absolute inset-0"></span>{{ featuredArticles[0].title }}</RouterLink></h3>
                <p class="mt-4 text-gray-500">{{ getExcerpt(featuredArticles[0].content, 150) }}</p>
              </div>
            </article>
            <div class="space-y-8">
              <article v-for="article in featuredArticles.slice(1, 3)" :key="article.id" class="flex items-center gap-4 group relative rounded-xl border border-gray-200 p-4 bg-white">
                <div class="h-24 w-24 flex-shrink-0 bg-gray-200 rounded-lg overflow-hidden"><svg class="h-full w-full text-gray-400" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909m-18 3.75h16.5a1.5 1.5 0 0 0 1.5-1.5V6a1.5 1.5 0 0 0-1.5-1.5H3.75A1.5 1.5 0 0 0 2.25 6v12a1.5 1.5 0 0 0 1.5 1.5Z"/></svg></div>
                <div>
                  ...
                  <h3 class="text-lg font-semibold mt-1"><RouterLink :to="`/blog/articles/${article.slug}`" class="hover:text-green-700"><span class="absolute inset-0"></span>{{ article.title }}</RouterLink></h3>
                </div>
              </article>
            </div>
          </div>
        ...
        <div v-if="isAdmin || isSuperAdmin" class="my-12 text-center">
          <RouterLink to="/blog/create" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
        ...
        <div class="mt-16">
          <div class="border-b border-gray-200 pb-5 mb-8">
            <div class="flex flex-wrap items-center justify-between gap-4">
              <h2 class="text-2xl font-bold tracking-tight text-gray-900">Explorer nos articles</h2>
              <div class="flex items-center gap-x-2 sm:gap-x-4">
                <div class="relative">
                  <input type="text" v-model="searchQuery" placeholder="Rechercher..." class="w-32 sm:w-48 pl-10 pr-4 py-2 text-sm rounded-full border-gray-300 focus:border-green-500 focus:ring-green-500">
                ...
                <select v-model="selectedCategory" class="text-sm rounded-full border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500 pl-3 pr-8 py-2"><option :value="null">Catégories</option><option v-for="category in categories" :key="category.id" :value="category.id">{{ category.name }}</option></select>
                <label class="flex items-center text-sm font-medium text-gray-700 cursor-pointer"><input type="checkbox" v-model="showFeaturedOnly" class="h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500 mr-2">À la une</label>
              </div>
            </div>
          </div>

          <div class="max-w-lg mx-auto grid gap-8 lg:grid-cols-3 lg:max-w-none">
            <div v-if="filteredArticles.length === 0" class="lg:col-span-3 text-center text-gray-500 py-16">Aucun article ne correspond à vos critères.</div>
            <article v-for="article in filteredArticles" :key="article.id" class="flex flex-col rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1 bg-white">
              <div class="flex-shrink-0 h-48 bg-gray-200 flex items-center justify-center relative">
              ...
              <div v-if="isAdmin || isSuperAdmin" class="bg-gray-50 border-t p-3 flex justify-end items-center space-x-3">
                <RouterLink :to="`/blog/edit/${article.slug}`" class="text-xs font-medium text-gray-600 hover:text-black hover:underline">
                  Modifier
                </RouterLink>
                <button @click="toggleFeatured(article)" class="text-xs font-medium py-1 px-2 rounded" :class="article.isFeatured ? 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'">
                  {{ article.isFeatured ? 'Retirer' : 'À la une' }}
                </button>
                <button v-if="isSuperAdmin" @click="deleteArticle(article.slug, article.id)" class="text-xs font-medium py-1 px-2 rounded bg-red-100 text-red-800 hover:bg-red-200">
                  Supprimer
                </button>
              </div>
