      ...
      <div v-else class="mt-12">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <article v-if="featuredArticles[0]" class="lg:col-span-2 relative rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 group bg-white dark:bg-gray-800">
              ...
              <div class="p-8">
                ...
                <h3 class="text-2xl font-bold leading-tight mt-2"><RouterLink :to="`/blog/articles/${featuredArticles[0].slug}`" class="hover:text-green-700 dark:hover:text-green-500 text-gray-900 dark:text-white"><span class="absolute inset-0"></span>{{ featuredArticles[0].title }}</RouterLink></h3>
                <p class="mt-4 text-gray-500 dark:text-gray-400">{{ getExcerpt(featuredArticles[0].content, 150) }}</p>
              </div>
            </article>
            <div class="space-y-8">
              <article v-for="article in featuredArticles.slice(1, 3)" :key="article.id" class="flex items-center gap-4 group relative rounded-xl border border-gray-200 dark:border-gray-700 p-4 bg-white dark:bg-gray-800">
                <div class="h-24 w-24 flex-shrink-0 bg-gray-200 dark:bg-gray-700 rounded-lg overflow-hidden">
                  <img
                ...
                <div>
                  ...
                  <h3 class="text-lg font-semibold mt-1 text-gray-900 dark:text-white"><RouterLink :to="`/blog/articles/${article.slug}`" class="hover:text-green-700 dark:hover:text-green-500"><span class="absolute inset-0"></span>{{ article.title }}</RouterLink></h3>
                </div>
              </article>
            </div>
          </div>
        ...
        <div class="mt-16">
            ...
            <div class="flex flex-col sm:flex-row flex-wrap items-start sm:items-center justify-between gap-4">
              ...
              <div class="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                <div class="relative w-full sm:w-auto mb-2 sm:mb-0">
                  <input type="text" v-model="searchQuery" placeholder="Rechercher..." class="w-full sm:w-32 md:w-48 pl-10 pr-4 py-2 text-sm rounded-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 focus:border-green-500 focus:ring-green-500 dark:focus:border-green-400 dark:focus:ring-green-400">
                ...
                <select v-model="selectedCategory" class="text-sm rounded-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 dark:focus:border-green-400 dark:focus:ring-green-400 pl-3 pr-8 py-2 mb-2 sm:mb-0 w-full sm:w-auto"><option :value="null">Catégories</option><option v-for="category in categories" :key="category.id" :value="category.id">{{ category.name }}</option></select>
                <select v-model="selectedYear" class="text-sm rounded-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-green-500 focus:ring-green-500 dark:focus:border-green-400 dark:focus:ring-green-400 pl-3 pr-8 py-2 mb-2 sm:mb-0 w-full sm:w-auto">
                  <option :value="null">Année</option>
                  <option v-for="year in archiveYears" :key="year" :value="year">{{ year }}</option>
                </select>
                <label class="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer w-full sm:w-auto"><input type="checkbox" v-model="showFeaturedOnly" class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-green-600 focus:ring-green-500 mr-2">À la une</label>
              </div>
            </div>
          </div>
          ...
          <div class="max-w-lg mx-auto grid gap-8 lg:grid-cols-3 lg:max-w-none">
            ...
            <article v-for="article in filteredArticles" :key="article.id" class="flex flex-col rounded-lg shadow-md dark:shadow-gray-900/30 overflow-hidden transition-all duration-300 hover:shadow-xl dark:hover:shadow-gray-900/50 hover:-translate-y-1 bg-white dark:bg-gray-800">
